...
const hunter = createEntity(15, false);
...
const loop = () => {
    // Dragão segue o mouse (mais rápido)
    update(dragon, pointer.x, pointer.y, 0.2, 12);
    
    // Hunter segue a ponta da cauda do dragão (mais lento)
    const tail = dragon[dragon.length - 1];
    update(hunter, tail.x, tail.y, 0.1, 8);
